...
Para ello, vamos a realizar varios ejercicios a partir de la obtención de datos públicos sobre personajes de Star Wars a través de una llamada fetch a la siguiente URL: https://swapi.dev/api/people/.
Este endpoint devuelve un objeto JSON que contiene información sobre diferentes personajes de Star Wars, con propiedades como nombre, género, altura, peso, color de cabello, color de ojos, planeta natal, año de nacimiento y más.

Se nos pide trabajar sobre este fichero T3-T4.js (que se carga mediante un script en el html del fichero T3-T4.html, en este mismo directorio) para:

 * 1.- Crear una función (a la que denominaremos "modifyPageNumberInput") que añada eventos al INPUT del formulario cuya id es "pageNumber" de modo que, al cambiar, lance la función "fetchSWAPIPepople" (que se describe más adelante) 
 pasando como parámetro el valor de este INPUT. 

 * 2.- Crear una función (a la que denominaremos "fetchSWAPIPepople") que realice una llamada fetch a a la API SWAPI (The Star Wars Api) para obtener un listado de todos los personajes recibidos por parámetro mediante el método GET 
  y el uso de bloques then() en JavaScript:
...
Per fer-ho, treballarem amb diversos exercicis a partir de l'obtenció de dades públiques sobre personatges de Star Wars a través d'una cridada fetch a la URL següent: https://swapi.dev/api/people/.
Aquest endpoint retorna un objecte JSON que conté informació sobre diferents personatges de Star Wars, amb propietats com a nom, gènere, altura, pes, color de cabell, color d'ulls, planeta natal, any de naixement i més.

Se'ns demana treballar sobre aquest fitxer T3-T4.js (que es carrega emprant un script al fitxer T3-T4.html, en aquest mateix directori) per a:

 * 1.- Crear una funció (a la que anomenarem "modifyPageNumberInput") que afegeixi esdeveniments a l'INPUT del formulari l'id del qual és "pageNumber" de manera que, en canviar, llanci la funció "fetchSWAPIPepople" (que es descriu més endavant)
 passant com a paràmetre el valor d'aquest INPUT.

 * 2.- Crear una funció (a la que anomenarem "fetchSWAPIPepople") que realitzi una cridada fetch a l'API SWAPI (The Star Wars Api) per obtenir un llistat de tots els personatges rebuts per paràmetre mitjançant el mètode GET
i l'ús de blocs then() a JavaScript:
  
    2.1- Abans de fer la cridada: Aquesta funció rebrà un número de pàgina per paràmetre i realitzarà una cridada fetch a l'API de SWAPI per obtenir un llistat dels personatges de StarWars que apareixen en aquesta pàgina.
    Així i tot, abans d'aquesta cridada, la funció ha de realitzar les accions següents:
    
        2.1.1- Ocultar el formulari (un form la id del qual és "myContent"), això ho farem afegint la classe "hidden" que ja ens ve definida al css. De manera que aquest element no es mostrarà mentre
        executem la funció i no haguem obtingut resultats encara.

        2.1.2.- Mostrar el carregador (un div la id del qual és "loader") això ho farem eliminant la classe "hidden", de manera que el carregador es mostri mentre executem la funció i no haguem obtingut resultats encara.
        Així, l'usuari podrà apreciar per pantalla que s'està trucant a l'API.
    ...
    Després, recollirem la resposta de l'anomenada fetch utilitzant un bloc then(), on, una vegada obtinguda la resposta, farem aquesta sèrie d'accions:
        ...
        2.2.2.- Convertirem la resposta a format JSON (utilitzant el mètode response.json()) en una constant definida dins de la funció el nom de la qual podeu triar lliurement.

        2.2.3.- Recorrerem l'Array de resultats que es troba a la propietat "results" de l'objecte obtingut després de la cridada (i que heu emmagatzemat anteriorment en una constant) i crearem un nou objecte del DOM,
        en aquest cas un SELECT, les OPTIONS del qual estaran conformades pel valor de la propietat "name" de cada personatge al TEXT i al VALUE de l'OPTION li assignarem la propietat "url" de cada personatge.

        2.2.4.- Ocultar el carregador (el div l'id del qual és "loader") això ho farem afegint la classe "hidden".

        2.2.5.- Mostrar el formulari (el form la id del qual és "myContent"), això ho farem eliminant-li la classe "hidden".

        2.2.6.- Afegir esdeveniments al SELECT del formulari creat de manera que, en canviar, modifiqueu l'ACTION del formulari amb el VALUE que teniu emmagatzemant l'OPTION seleccionat (usar dispatchEvent).

* 3.- Crear una funció (a la que anomenarem modifySendFormButton) que empri addEventListener perquè, quan es cliqui al botó l'id del qual és "sendForm" (i que és el que en principi enviaria el formulari), no es realitzi l'acció
